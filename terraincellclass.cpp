...
bool TerrainCellClass::InitializeBuffers(ID3D11Device* device, int nodeIndexX, int nodeIndexY, int cellHeight, int cellWidth, int terrainWidth, ModelType* terrainModel)
{
	...
	//Setup the indexes into the tarrain model data and local vertex/index array
	...
	for (int i = 0; i < (cellHeight - 1); i++)
	{
		for (int j = 0; j < ((cellWidth - 1) * 6); j++)
		{
			...
			vertices[index].color = XMFLOAT4(terrainModel[modelIndex].r, terrainModel[modelIndex].g, terrainModel[modelIndex].b, 1.0f);
			indices[index] = index;
			modelIndex++;
			index++;
		}
	...
	m_vertexList = new XMFLOAT3[m_vertexCount];
	if (!m_vertexList)
	{
		return false;
	}
	...
	//Release the array now that the buffers have been created and loaded
	delete[] vertices;
	vertices = nullptr;
...
void TerrainCellClass::CalculateCellDimensions()
{
	...
	//Initalize the dimensions of the node
