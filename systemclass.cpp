...
void SystemClass::Run()
{
	...
	while (!done)
	{
		...
		else if (m_Input->IsEscapePressed())
		{
			done = true;
		}
...
bool SystemClass::Frame()
{
	...
	result = m_Input->Frame(m_Timer->GetTime(), posX, posY, posZ, rotX, rotY, rotZ);
	if (!result)
	{
		return false;
	}
	...
	result = m_Application->Frame(m_Input, m_Timer->GetTime(), posX, posY, posZ, rotX, rotY, rotZ);
	if (!result)
	{
		return false;
	}
...
void SystemClass::InitializeWindows(int& screenWidth, int& screenHeight)
{
	...
	//Get the instanze of this application
	...
	//Setup the screen settings depennding on whether it is running in full screen or in windowed mode
	if (FULL_SCREEN)
	{
		//Ìf full screen set the screen to maximum size of the users desktop and 32bit
