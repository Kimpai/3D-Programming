	...
	float screenDepth, float screenNear)
{
	...
	//Get the number of modes that fit the DXGI_FORMAT_R8G8B8A8_UNORM disålay format for the adapter output
	...
	when a match is found store the numerator and demoninator of the refresh rate for that monitor*/
	...
	//Set the full screen or windowed mode
	if (fullscreen)
	{
	...
	//Discard the back buffer after contents is displayed
	swapChainDesc.SwapEffect = DXGI_SWAP_EFFECT_DISCARD;

	//Don't set advanced flags
	swapChainDesc.Flags = 0;

	//Set the feature level of DirectX 11
	...
	//Initialize the desc of the depth buffer
	ZeroMemory(&depthBufferDesc, sizeof(depthBufferDesc));

	//Set up the desc of the depth buffer
	...
	//Initialize the desc for the stencil state
	...
	//Setup the raster desc wich will determine how polygons are drawn
	...
	//Create the raster state from the desc
	...
	//Now set the raster state
	m_deviceContext->RSSetState(m_rasterState);

	//Setup a raster desc which turns off back face culling
	rasterDesc.AntialiasedLineEnable = false;
	...
	//Clear the second depth stencil state wihch turns off the Z buffer for 2D rendering
	...
	//Modify the desc to create an alpha disabled blend state desc
...
{
	//Bind the render target view and depth stencil state buffer to the output render pipeline
	m_deviceContext->OMSetRenderTargets(1, &m_renderTargetView, m_depthStencilView);
...
void D3DClass::SetVideoCardInfo(char* cardName, int& memory)
{
